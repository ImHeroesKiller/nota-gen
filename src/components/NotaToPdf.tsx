...
interface NotaToPdfProps {
  onBack: () => void;
  darkMode: boolean;
  setDarkMode: (value: boolean) => void;
}

export default function NotaToPdf({ onBack, darkMode, setDarkMode }: NotaToPdfProps) {
          ...
          <div className={`w-full max-w-lg rounded-2xl p-6 shadow-2xl ${darkMode ? 'bg-[#161b22] border border-[#30363d]' : 'bg-white border border-[#e2e5e9]'}`} onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4 text-sm leading-relaxed">
              <p className={textSecondary}>
                <strong className={textPrimary}>PERADA GROUP</strong> menghadirkan kekuatan gabungan antara keahlian logistik dan pengelolaan sumber daya manusia serta operasional bisnis.
              </p>
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-auto preview-scroll p-6 flex items-start justify-center">
            {images.length === 0 ? (
              ...
              <PreviewPage images={images} settings={settings} currentPage={currentPage} zoom={zoom} />
            )}
          </div>
